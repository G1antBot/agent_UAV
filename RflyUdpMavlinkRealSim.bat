...
@REM REM Check if DLLModel is a name string, if yes, copy the DLL file to CopterSim folder
...
@REM CameraRoom:动捕模式; OldFactory:旧工厂; NeighborhoodPark:社区公园; 
SET UE4_MAP=ChallengeMap
@REM SET UE4_MAP=NeighborhoodPark
...
REM Keep 1m interval as original
...
ECHO.
ECHO ---------------------------------------
@REM REM Max vehicle number 50
@REM SET /a MAX_VEHICLE=50
SET /a VehicleNum=1
REM 4 independent UAVs
...
REM Configure RflySim3D view code
timeout /t 5 /nobreak > nul

start %PSP_PATH%\Python38\python.exe "%~dp0\generate.py"
