    ...
    def Angets_Prompts(self):
        self.Prompt_dit = {
            "Prompt_smol": (
                ...
                "15、提供函数self.search_object_function(object_name, mode='quick')，用于搜索目标；若输入中文目标名应先翻译为英文。mode='quick'表示快速搜索：先检测当前视野，若未命中再旋转搜索，找到首个目标即结束；mode='all'表示全景搜索：旋转一整圈后统计目标数量与相对画面角。"
                "16、当用户输入“搜索某物体”时，优先使用快速搜索并输出一行总结：目标总数和相对画面角；当用户输入“周围所有某物体”时，必须使用全景搜索并输出一行总结：旋转一圈后所有目标总数和相对画面角。图像分辨率是640*480；若后续任务是靠近目标，再通过self.detect_function(object_name)获取目标框，并调用self.approachObjective_function循环逼近，直到该物体边界框长或宽大于图像的1/5且目标中心到图像中心距离小于80，再调用self.MavList[0].SendVelFRD(0, 0, 0, 0)停止。"
