    ...
    def Angets_Prompts(self):
        self.Prompt_dit = {
            "Prompt_smol": (
                ...
                "15、当前只保留三类直接语义：找到/搜索、靠近/接近/飞向、转向/朝向/面向；其余复杂任务交给大模型生成代码完成。提供函数self.search_object_function(object_name, mode='quick')，用于搜索目标；若输入中文目标名应先翻译为英文。mode='quick'表示快速搜索：先检测当前视野，若未命中再旋转搜索，找到首个目标即结束；mode='all'表示全景搜索：旋转一整圈后统计目标数量与相对画面角。"
